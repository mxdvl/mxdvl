<script lang="ts">
	...
	const HALF = MARATHON / 2;

	function round(value: number, nearest: number) {
	...
	<table>
		...
		<tbody>
			...
			<tr>
				<td>Half</td>
				<td colspan="2">
					<button onclick={() => perKilometre = round(perKilometre * HALF - 20, 20) / HALF}>-</button>
					{format(HALF * perKilometre)}
					<button onclick={() => perKilometre = round(perKilometre * HALF + 20, 20) / HALF}>+</button>
				</td>
			</tr>
			<tr>

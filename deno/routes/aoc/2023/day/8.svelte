<script>
  import NetworkGraph from "../../../../components/aoc/NetworkGraph.svelte";
    ...
    (acc, steps, group) => {
      ...
      for (const step of unique_steps) {
        ...
        const center = ({ group }) => ({
          ...
          y:
            Math.floor(group / 3) * ((4 * height) / part_two_steps.length) -
            height / 2,
        });
        ...
        const dx =
          radius * Math.sin((Math.PI * 2 * step_index) / unique_steps.size);
        const dy =
          radius * Math.cos((Math.PI * 2 * step_index) / unique_steps.size);
        acc.nodes.push({ id: step, group, x: x + dx, y: y + dy });
        acc.links.push(
          { source: step, target: node.L },
          { source: step, target: node.R },
        );
...
<h2>
  Part two: {lcmAll(part_two_steps.map((steps) => steps.length - 1))} steps
</h2>

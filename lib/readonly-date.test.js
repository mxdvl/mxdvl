...
Deno.test({
	...
	async fn({ step }) {
		...
		await step("year, month (day defaults to 1)", () => {
			const date = new ReadonlyDate(2024, 5);
			assertEquals(date.getUTCFullYear(), 2024);
			assertEquals(date.getUTCMonth(), 5); // June
			assertEquals(date.getUTCDate(), 1); // defaults to 1
		});
	},
});
